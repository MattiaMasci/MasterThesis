...
class Conv2Net(nn.Module):
    ...
    def weights_init(self, first, second, third, fourth):
        ...
        self.net[7].weight = nn.Parameter(third, requires_grad=True)
        
        # Initialization of second linear layer weights
        self.net[9].weight = nn.Parameter(fourth, requires_grad=True)
    ...
    def train(self, dataloaders, learning_rate=1e-3, loss_fn=nn.functional.cross_entropy, epochs=50):
        ...
        # influence Analysis
        torch.save(accuracy_analysis_array, '../../plot/basicModel/influenceAnalysis/conv2Net/accuracy50.pt')
        torch.save(loss_analysis_array, '../../plot/basicModel/influenceAnalysis/conv2Net/loss50.pt')
