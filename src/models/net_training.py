...
for t in range(epochs):
    ...
    grad_dict = train_loop(train_dataloader, net, loss_fn, optimizer)
...
# normalizedGradientDifferenceFreezingProcedure
torch.save(freezing_rate_values, '../../plot/data/freezingRateProcedure/freezing_rate50.pt')

"""
# gradientNormChangeFreezingProcedure
torch.save(gradient_difference_norm_change_array, '../../plot/data/gradientNormChanges/gradient_difference_norm_change50.pt')
torch.save(gradient_norm_difference_change_array, '../../plot/data/gradientNormChanges/gradient_norm_difference_change50.pt')

# influence Analysis
torch.save(accuracy_analysis_array, '../../plot/data/influenceAnalysis/accuracy50.pt')
torch.save(loss_analysis_array, '../../plot/data/influenceAnalysis/loss50.pt')
...
plt.plot(list(range(1, epochs+1)), freezing_rate_values, marker = 'o')
plt.legend(["conv1","conv2","conv3","conv4","conv5","fc1","fc2"], loc ="lower right", fontsize=10)
plt.savefig('../../plot/freezing_rate.png')

plt.clf()
plt.xlabel('Epoch')
plt.ylabel('% accuracy')
plt.plot(list(range(1, epochs+1)), net_acc_values*100, marker = 'o')
plt.savefig('../../plot/freezing_training_accuracy.png')

plt.clf()
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.plot(list(range(1, epochs+1)), net_loss_values, color='red', marker = 'o')
plt.savefig('../../plot/freezing_training_loss.png')
"""